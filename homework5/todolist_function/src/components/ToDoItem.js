...
function ToDoItem({delItems, complete, index, text}) {
    ...
    return (
        <Grid container spacing={1} justify="space-around" style={{ marginTop: "10px", border: "1px solid black", borderRadius: "10px 10px", padding: "2px" }}>
            <Grid item >  <Checkbox style={{ color: "green" }} onClick={()=>{setStatus(!status);complete()}}  /></Grid>
            <Grid item > <TextField id="filled-basic" value={text} style={status ? {textDecoration: "line-through"}:{textDecoration:"none"}} /></Grid>
